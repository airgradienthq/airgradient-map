<template>
  <div class="map-wrapper">
    ...
    <DialogsLocationHistoryDialog
      v-if="locationHistoryDialog"
      :dialog="locationHistoryDialog"
    />
  </div>
</template>
...
function setUpMapInstance(): void {
  ...
  mapInstance.on('movestart zoomstart', () => { isMapMoving.value = true; });
  mapInstance.on('moveend zoomend', () => { isMapMoving.value = false; });
...
@media (max-width: 779px) {
  .leaflet-control-geosearch { display: flex; }
