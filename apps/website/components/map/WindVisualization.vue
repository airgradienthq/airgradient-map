...
function pixelToLatLng(x: number, y: number) {
  const lat = props.bounds.north - (y / canvasHeight.value) * (props.bounds.north - props.bounds.south)
  const lng = props.bounds.west + (x / canvasWidth.value) * (props.bounds.east - props.bounds.west)
  return { lat, lng }
}
